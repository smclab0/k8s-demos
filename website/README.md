...
RBAC (`rbac.yaml`): the `website` ServiceAccount can `list` Pods and `list` Nodes (cluster-scoped, since nodes aren't namespaced) -- both read-only, same narrow-grant pattern `trading/dashboard-rbac.yaml` uses -- plus (as of the "chaos to madness" button below) `patch` its own Deployment's `/scale` subresource, `resourceNames`-restricted to `website`. Talking to Redis doesn't need RBAC -- it's a plain Service connection, not a k8s API call.

### Chaos to madness

A button in the header panel: `POST /api/madness` fires 10,000 requests at `website`'s ClusterIP (a `ThreadPoolExecutor`, 80 workers, inside the `api` sidecar) and scales it to 100 replicas, holds 60s, scales back to 3 -- all in a background thread, so the click returns immediately rather than hanging the browser for however long 10,000 requests take. State (`running`/`sent`/`total`/`phase`) lives server-side and is polled (`GET /api/madness`, every 700ms while active), not tracked client-side -- so every open tab shows the same live progress, including ones that didn't click the button, and a second click while one's already running is rejected (409) rather than stacking a second run on top.

Verified the state machine locally (mocked `scale_website`/the request-firing function, ran the real threading/locking code): concurrent `start_madness()` calls while one's active correctly return `False`; the real one runs scale-up -> fire all N -> hold -> scale-down in order and resets `running` to `False` at the end.
