#!/usr/bin/env bash
# Deploys the website demo with the Ingress host set to a live sslip.io
# FQDN derived from the cluster's own traefik LoadBalancer IP, resolved
# fresh from the cluster at apply time. Usage: website/deploy.sh
set -euo pipefail
...
FQDN="website.${TRAEFIK_IP}.sslip.io"
DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"

kubectl kustomize "${DIR}" | sed "s/website\.example\.com/${FQDN}/g" | kubectl apply -f -
